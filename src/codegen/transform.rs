...
impl<'a> LowerToLlvm<'a> {
    ...
    unsafe fn get_type(&mut self, ty: &monoir::Type, param: bool) -> LLVMTypeRef {
        ...
        match *ty {
            ...
            Function {
                ...
                let mut params_ty: Vec<LLVMTypeRef> = params_ty
                    .iter()
    ...
    pub fn gen_extern(&mut self, sym: &monoir::Symbol) -> Result<()> {
        ...
        let _ = unsafe { Prelude::emit(sym, func, self.module, self.builder)? };
        Ok(())
    }
    ...
    fn add_bb(&mut self, func: LLVMValueRef, name: &str) -> LLVMBasicBlockRef {
        unsafe { LLVMAppendBasicBlockInContext(*self.context, func, label(name).as_ptr()) }
    }
